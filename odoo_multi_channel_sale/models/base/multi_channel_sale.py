...
class MultiChannelSale(models.Model):
	...
	def get_quantity(self, obj_pro):
		...
		elif self.channel_stock_action == "qa":
	...
	delay_pas_order_change = fields.Integer(default=4,required=True,string="Durée recherche Commande modifiées (en heure)")
	export_images = fields.Boolean(default=True,string="Export Images")
	synchro_text = fields.Boolean(default=True, string="Synchronisaton des textes")
	...
	pricelist_regular = fields.Many2one(
		comodel_name='product.pricelist',
		string='Regular Pricelist',
		default=lambda self: self.env['product.pricelist'].search([], limit=1),
		help="""Select the same currency of pricelist used  over e-commerce store/marketplace.""",
	)
	...
	@api.model
	def cron_mapping_update(self):
		records = self.env["channel.template.mappings"].search([("need_sync","=","yes")])
		for record in records:
			res =  Transaction(channel=record.channel_id).export_data(
				object = "product.template",
				object_ids = [record.template_name.id,],
				operation = "update",
			)
			self.env.cr.commit()
		return True
