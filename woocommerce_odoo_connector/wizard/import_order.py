...
class ImportWoocommerceOrders(models.TransientModel):
    ...
    def get_order_all(self, woocommerce, channel, **kwargs):
        vals_list = []
        orders = woocommerce.get('orders', params={"page": kwargs.get("page"), "per_page": kwargs.get("page_size"), 'order': 'asc'}).json()
    ...
    def _filter_order_using_date(self, woocommerce, channel, **kwargs):
        vals_list = []
        date_created = None
        import_order_date = (kwargs.get("woocommerce_import_date_from") - timedelta(hours=channel.delay_pas_order_change)).isoformat()
    ...
    def _get_woocommerce_shipping(self, shipping_line, include_in_price, woocommerce, channel):
        shipping_list = []
        for shipping in shipping_line:
            shipping_line = {
                ...
                'line_taxes': self.env["import.woocommerce.taxes"].create({
                    "channel_id": channel.id,
                    "operation": "import"
                }).get_woocommerce_taxes(woocommerce, shipping.get("taxes"), include_in_price)
            }
    ...
    def _get_order_by_id(self, woocommerce, channel, order):
        ...
        if order['shipping_lines']:
            order_lines += self._get_woocommerce_shipping(
                order['shipping_lines'], include_in_price, woocommerce, channel)
        ...
        if order['shipping']:
            ...
            if order['meta_data']:
                for meta_data in order['meta_data']:
                    if meta_data["key"] == '_lpc_meta_pickUpLocationId':
                         order_dict.update({'shipping_code_relaypoint': meta_data['value']})
