...
class ProductTemplate(models.Model):
	...
	description_web = fields.Html(string='Description WEB')
	description_short_web = fields.Html(string='Description Courte WEB')
	images_ids = fields.One2many('product.images', 'product_tmpl_id', 'Images')
	manage_stock = fields.Boolean('Web Gestion Stock', compute="_compute_manage_stock", store=True)
	web_image_id = fields.Integer("Id Site WEB")

	@api.depends('type')
	def _compute_manage_stock(self):
		for product in self:
			product.manage_stock = True if product.type == 'product' else False
	...
	def get_regular_price(self, channel_id):
		self.ensure_one()
		if channel_id.pricelist_regular:
			prices = channel_id.pricelist_regular.get_products_price(self, [1.0], [None])
			if prices:
				return prices.get(self.id, 0.0)
		return self.price_list

	def get_standard_price(self, channel_id):
		self.ensure_one()
		if channel_id.pricelist_name:
			prices = channel_id.pricelist_name.get_products_price(self, [1.0], [None])
			if prices:
				return prices.get(self.id, 0.0)
		return self.get_standard_price(channel_id)
